...
def run(args):
    ...
    ##################
    # changed by Aaron
    # include the downloaded weights from the texture paper trained on StylizedImageNet
    # if using one of those sets of weights, set the architecture to resnet50
    # otherwise use the name of the architecture provided
    if args.arch_name in ["resnet50_combined_with_decay.pth.tar", "resnet50_combined_SIN_IN.pth.tar", "resnet50_SIN.pth.tar"]:
        net = models.resnet50()
    else:
    ...
    ##################
    # use given targets unless all_targets argument is passed, then use all 1000 in ImageNet or StylizedImageNet. currently all_targets not working
    if not args.all_targets:
        targets = [int(x) for x in args.targets.split(',')]
    else:
        targets = [x for x in range(999)]
    net = net.to(device)
    ...
    ### load models
    # changed by Aaron to use the SIN downloaded weights.
    # this is only necessary if the weights were downloaded and stored in the DI directory
    # if they are included in PyTorch, this should be ignored
    if args.arch_name in ["resnet50_combined_with_decay.pth.tar", "resnet50_combined_SIN_IN.pth.tar", "resnet50_SIN.pth.tar"]:
        path_to_model = f"./{args.arch_name}"
...
def main():
    ...
    ################
    # Added by Aaron
    parser.add_argument('--targets', type=str, default=None, help='which ImageNet classes to target')
    parser.add_argument('--stop_layer', type=int, help='pull batchnorm mean/var from all layers up til this one')
    parser.add_argument('--specified_layer', type=int, help='pull batchnorm mean/var only from this layer')
    parser.add_argument('--all_targets', action='store_true', help='set to store all 1000 IN/SIN targets')
    ################
