...
import collections
# not using apex for the project
...
class DeepInversionClass(object):
    def __init__(self, bs=84,
        ...
        # don't want to use stop_layer and specified layer together, they are two
        # mutually exclusive strategies

        # warning, this code causes a bug if you set both to 0.
        assert specified_layer or stop_layer, "Need either stop_layer or specified_layer > 0. These are not 0-indexed, layer 1 the first layer."
        if stop_layer:
            self.stop_layer = stop_layer
            specified_layer = None
        if specified_layer:
            self.stop_layer = None
            self.specified_layer = specified_layer
        ############################
        ...
        if local_rank==0:
            ...
            # commenting this out. -Aaron
            # thing was making 1000 empty folders each time and not using them -Aaron
            #for m in range(1000):
            #    create_folder(self.final_data_path + "/s{:03d}".format(m))

        
        ###############################################################
        # changing to stop at desired layers for texture/shape experiment - Aaron
        ## Create hooks for feature statistics
        self.loss_r_feature_layers = []
        
        for idx, module in enumerate(self.net_teacher.modules()):
            if isinstance(module, nn.BatchNorm2d):
                self.loss_r_feature_layers.append(DeepInversionFeatureHook(module))
            # note that this is cumulative through the stop layer - Aaron
            # not using stop layers for now.
            if self.stop_layer and idx == self.stop_layer:
                print(f"Confirming stopped at BN layer {self.stop_layer}")
                break
        
        # adding specified_layer functionality
        # this is not the most efficient way because I'm letting it build self.loss_r_feature_layers
        # but shouldn't make too much difference.
        bn_layer_ct = len(self.loss_r_feature_layers)

        if self.specified_layer and not stop_layer:
            assert specified_layer <= bn_layer_ct, f"specified_layer must be <= {bn_layer_ct} for this {self.net_teacher.__class__.__name__} model"
            
            # this needs to stay a list or the original code will throw an error
            # I've dealt with the 0-indexing bug here by starting at specified_layer-1
            # Experiments 6 and below didn't have this change, so they are 0-indexed.
            self.loss_r_feature_layers = self.loss_r_feature_layers[self.specified_layer-1:self.specified_layer]
            print(f"Confirming BN layer {self.specified_layer} specified")
        
        self.hook_for_display = None
        if hook_for_display is not None:
            self.hook_for_display = hook_for_display
        ###############################################################
    def get_images(self, net_student=None):
        ...
        ##################################################################################
        # setup target labels
        # changed by Aaron, so we can specify targets in shell script
        ...
        if not self.random_label:
            ...
            # Kefeng suggests that this will not change the actual targets, just the 
            # number of images per batch.
            # if so, bs must be a multiple of the number of targets.
            assert self.bs % len(targets) == 0, "Batch size must be a multiple of the number of targets."
            targets = torch.LongTensor(targets * (int(self.bs / len(targets)))).to('cuda')

            # not super important.
            # load ImageNet class name dict from file, then print which classes you've selected
            with open("imagenet_class_dict.txt") as f:
                class_dict = dict(ast.literal_eval(f.read()))
            if len(self.targets) < 30:
                print("------------Target classes----------")
                for c in self.targets:
                    print(f"class {c}: {class_dict[c]}")
        ###################################################################################    
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                #############################################################
                # R_feature loss
                # important for our experiment texture vs shape
                # haven't changed this yet, instead opted to change how self.loss_r_feature_layers is formed (above)

                # not sure if we want this rescaling, leaving alone for now
                # should rework this whole thing when tuning actually. right now with the single layer it's just interfering with 
                # bn_reg_scale. essentially bn reg scale is 10 times whatever it's set at because of this. but
                # when I rework this, need to find a way to keep both stop_layer and specified_layer functional...
    ...
    def save_images(self, images, targets):
        ...
        # I don't understand their code here. Why do they have the useless "if 0:" statement? and why is the only difference having an s folder in front?
        local_rank = torch.cuda.current_device()
        for id in range(images.shape[0]):
            class_id = targets[id].item()

            # commenting out this weird bit - Aaron
            #if 0:
    ...
    def generate_batch(self, net_student=None):
        ...
        self.get_images(net_student=net_student)
