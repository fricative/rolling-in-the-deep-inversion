# --------------------------------------------------------
# Modified by Kefeng Zhu
# Implementation of the Class-Conditional Regularization
# On 2021.07.12 Mon 18:00:18 CST
# --------------------------------------------------------
...
class DeepInversionFeatureHook():
    ...
    def hook_fn(self, module, input, output):
        ...
        stats_var = torch.from_numpy(self.stats['var']).to('cuda').data.type(var.type())
        stats_mean = torch.from_numpy(self.stats['mean']).to('cuda').data.type(mean.type())
        r_feature = torch.norm(stats_var - var, 2) + torch.norm(stats_mean - mean, 2)
...
class DeepInversionClass(object):
    def __init__(self, bs=84,
                 ...
                 target_label=0, labeled_stats=None):
        '''
        ...
        :param coefficients: dictionary with parameters and coefficients for optimization.
            ...
            "labeled_reg_feature" - coefficient for labeled feature distribution regularization 
                - kfz on Thu Sep  3 01:47:09 UTC 2020
        ...
        hook_for_display: function to be executed at every print/save call, useful to check accuracy of verifier
            ...
            "target_label": the target label that labeled_reg_feature need to specify
            "labeled_stats": label-speicified statistics data
        '''

        # labeled reg feature required parameters - kfz on
        self.target_label = target_label
        self.labeled_stats = labeled_stats
        ...
        if "r_feature" in coefficients:
            ...
            self.labeled_reg_scale = coefficients["labeled_reg_feature"]
        else:
        ...
        # changed by tong 2020.9.22
        prefix_label = os.path.join(prefix, '{}'.format(self.target_label))
        final_data_label = os.path.join(self.final_data_path, '{}'.format(self.target_label))
        local_rank = torch.cuda.current_device()
        if local_rank==0:
            #create_folder(prefix)
            #create_folder(prefix_label)
            #create_folder(prefix + "/best_images/")
            create_folder(self.final_data_path)
            create_folder(final_data_label)
        ...
        for module in self.net_teacher.modules():
            if isinstance(module, nn.BatchNorm2d):
                ...
                self.loss_r_feature_layers.append(DeepInversionFeatureHook(module, self.labeled_stats[self.target_label][idx]))
                idx += 1
    ...
    def get_images(self, net_student=None, targets=None):
        ...
        # NEW METHOD 
        #--------------------------------------------------------------------------------
        # set the pre-determined target label for images to be generated
        targets = torch.LongTensor([self.target_label for _ in range(self.bs)]).to('cuda')
        #--------------------------------------------------------------------------------
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                #--------------------------------------------------------------------------------
                # NEW METHOD - added by KFZ on Thu Sep  3 02:18:44 UTC 2020
                # TODO: Loss from New Method on ImageNet - labelized regularization term 
                #--------------------------------------------------------------------------------
                if self.labeled_reg_scale != 0.0 and not self.labeled_stats is None:
                    loss_labeled_feature_list = []
                    ct = 0
                    for mod, stats in zip(self.loss_r_feature_layers, self.labeled_stats[self.target_label]):
                        stats_mean = torch.from_numpy(stats['mean']).to('cuda').data.type(mod.mean.type())
                        stats_var = torch.from_numpy(stats['var']).to('cuda').data.type(mod.var.type())

                        # The new regularization term - a labeled version of r_feature
                        #if ct == 0:
                        #tmp_loss = torch.norm(mod.mean - stats_mean, 2) + torch.norm(mod.var - stats_var, 2)
                        tmp_loss = kl_loss(torch.log(mod.mean), stats_mean) + kl_loss(torch.log(mod.var), stats_var)
                        loss_labeled_feature_list.append(tmp_loss)
                        #print("layer %02d - calc-mean: %10.3f, stats-mean: %10.3f;\t calc-var: %10.3f, stats-var: %10.3f" 
                        #        % (ct, torch.mean(mod.mean).item(), torch.mean(stats_mean).item(), torch.mean(mod.var).item(), torch.mean(stats_var).item()))
                        ct += 1

                    loss_labeled_feature = sum(loss_labeled_feature_list)
                    #loss = loss + labeled_reg_scale*loss_labeled_feature 
                #================================================================================
                ...
                if self.labeled_reg_scale != 0.0 and not self.labeled_stats is None:
                    loss_aux += self.labeled_reg_scale * loss_labeled_feature
                ...
                if local_rank==0:
                    if iteration % save_every==0:
                        ...
                        if self.labeled_reg_scale != 0.0 and not self.labeled_stats is None:
                            print("loss_labeled_feature", loss_labeled_feature.item())
                ...
                # changed by tong, purpose: save the last generated pics
                #if iteration_loc == (iterations_per_layer-1):
                #    current_time = time.strftime("%Y%m%d%H%M%S", time.localtime())
                #    print(current_time)
                #    if local_rank==0:
                #        for i in range(self.bs):
                #            vutils.save_image(inputs.data.clone()[i],
                #                          '{}/{}/{}_{:05d}_{}_gpu_{}.png'.format(self.prefix, self.target_label,
                #                                                                 current_time,
                #                                                                 iteration // save_every, i, 
                #                                                                 local_rank),
    ...
    def save_images(self, images, targets):
        ...
        for id in range(images.shape[0]):
            ...
            if 0:
                ...
                #place_to_store = '{}/s{:03d}/{}/img_{:05d}_id{:03d}_gpu_{}_2.jpg'.format(self.final_data_path, 
                place_to_store = '{}/s{:03d}/{}/{}_img_{:05d}_id{:03d}_gpu_{}_2.jpg'.format(self.final_data_path, 
                                                                                         class_id, self.target_label,
                                                                                         time.strftime("%Y%m%d%H%M", time.localtime()),
                                                                                          self.num_generations, id,
                                                                                          local_rank)
            else:
                place_to_store = '{}/{}/img_s{:03d}_{:05d}_id{:03d}_gpu_{}_2.jpg'.format(self.final_data_path, 
