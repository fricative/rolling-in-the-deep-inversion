...
def run_deep_inversion(
    ...
    for epoch in range(epochs):
        ...
        if epoch >= 1995:
          base_number = (epoch - 1995 ) * bs 
          generated_imgs = [best_inputs[i, :] for i in range(best_inputs.shape[0])]
          for i, img in enumerate(generated_imgs):
            vutils.save_image(img, os.path.join(prefix, '{}.png'.format(i + base_number)), normalize=True)
